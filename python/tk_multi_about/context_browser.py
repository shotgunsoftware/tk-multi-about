...
class ContextBrowserWidget(browser_widget.BrowserWidget):
    ...
    def get_data(self, data):
        ...
        if ctx.project:
            # get project data
            data["project"] = self._app.shotgun.find_one(
                "Project",
        ...
        if ctx.entity:
            # get entity data
            data["entity"] = self._app.shotgun.find_one(
                ctx.entity["type"],
                [["id", "is", ctx.entity["id"]]],
        ...
        if ctx.step:
            # get step data
            data["step"] = self._app.shotgun.find_one(
                "Step", [["id", "is", ctx.step["id"]]], ["code", "description"]
            )

        if ctx.task:
            # get task data
            data["task"] = self._app.shotgun.find_one(
                "Task",
                [["id", "is", ctx.task["id"]]],
        ...
        for ae in ctx.additional_entities:
            ...
            d = self._app.shotgun.find_one(
                ae["type"], [["id", "is", ae["id"]]], ["code", "description", "image"]
            )
    ...
    def process_result(self, result):
        ...
        if result.get("project"):
            ...
            details.append(
                "<b>Project %s</b> (%s)" % (d.get("name"), site_url_nice_name)
            )
            details.append(
                d.get("sg_description") if d.get("sg_description") else "No Description"
            )
        ...
        if result.get("entity"):
            ...
            i = self.add_item(browser_widget.ListItem)
            details = []
            nice_name = sgtk.util.get_entity_type_display_name(
                self._app.sgtk, d.get("type")
            )
            details.append("<b>%s %s</b>" % (nice_name, d.get("code")))
            details.append(
                d.get("description") if d.get("description") else "No Description"
            )
            i.set_details("<br>".join(details))
            i.sg_data = d
            i.setToolTip("Double click to see more details in Shotgun.")
